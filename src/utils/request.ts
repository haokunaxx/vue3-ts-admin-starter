import axios, {
  AxiosRequestConfig, AxiosResponse, AxiosInstance, Axios
} from 'axios'
...
export class Request {
  ...
  constructor(config: AxiosRequestConfigWithInterceptors) {
    ...
    // 类请求拦截器（通用）
    interceptors.request.use((config: AxiosRequestConfig): AxiosRequestConfig => {
    ...
    // 类响应拦截器 (通用)
    interceptors.response.use((res: AxiosResponse<TodoResponse>) => {
  ...
  cancelRequest(url: string | string[]) {
    if (Array.isArray(url)) {
      url.forEach(requestNeedCancel => {
        const item = this.requestCancelPool.find(remainRequest => remainRequest[requestNeedCancel])
        item && item[requestNeedCancel]()
      })
    } else {
      const item = this.requestCancelPool.find(remainRequest => remainRequest[url])
      item && item[url]()
    }
  }
}
